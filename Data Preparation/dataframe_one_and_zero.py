#converts chicago.txt to a sparce dataframe with 0 if the feature does not exist and 1 if it does
		...
		for word in line_words:
			for index in range(0,256):
				...
				if word.isnumeric() and df.columns[index] == int(word) and len(word) == 3: 
					df.at[rowCount,df.columns[index]] = 1
...
df.to_csv(r'chicago_dataframe_one_and_zero.txt', index=None, sep='\t', mode='w')